...
*Packages Load*
```{r package load: su,warning=FALSE}
# load packages
...
library(tigris)        # to access US geographic data 
library(e1071)         # For skewness calculation
library(randomForest)  # For bilding random forest models
...
na_counts_tbl <- as_tibble(na_counts_df)
...
high_cor_indices <- which(abs(cor_matrix) > 0.7, arr.ind = TRUE) # 0.8 is more commonly accepted, also run regression andview variance inflation factors with logistic regression
...
high_cor_pairs_tbl <- as_tibble(high_cor_pairs)

# Print the tibble
print(high_cor_pairs_tbl)
...
```{r per cent poor to fair vs per cent voting: su}
...
```{r per cent poor to fair vs school funding gap: su}
# Create the scatter plot with x-axis limit, percentage formatting, and state labels
ggplot(data = qol_data,
...
```{r per cent poor to fair vs years of life: su}
# Create the scatter plot with x-axis limit, percentage formatting, and state labels
ggplot(data = qol_data,
...
*Table creation with gtsummary*

```{r race specific data frames}
...
st <- race_stats_calc(data = qol_data_long,
                      race = df_race_list,
...
```{r function creation attempts 2}
# create function
race_stats_calc <- function(data, race_list, feature_list) {
  summary_stats <- data %>%
...
```{r filter data by race}

qol_data_na <- qol_data_na %>% 
  mutate(pct_poor_to_fair_health = pct_poor_to_fair_health * percentage)


native_american <- summary(qol_data_na$pct_voters)
asian <- summary(qol_data_asian$pct_voters)
black <- summary(qol_data_black$pct_voters)
hispanic <- summary(qol_data_hispanic$pct_voters)
white <- summary(qol_data_white$pct_voters)
other <- summary(qol_data_other$pct_voters)
...
###Perform arithmetic transformations for skewed features
```{r}
...
###normalize and or scale features
...
model_rf <- randomForest(as.factor(response) ~ ., data = qol_train, ntree = 500)
...
roc_obj <- roc(qol_test$response, as.numeric(predictions2))
auc_rf <- auc(roc_obj)
print(auc_rf)
...
pca_model <- prcomp(qol_train_standardized, center = TRUE, scale. = TRUE) # apply PCA
