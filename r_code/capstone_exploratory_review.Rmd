...
Remove predictors with large correlation value (|0.7|) or greater with multicollinearity and low relavence.
...
```{r}
# Extract the summary
reg_summary <- summary(qol_regfit_ful)

# Get the matrix of which variables are included in each model
reg_summary_matrix <- reg_summary$which
...
# Filter for models with decreasing numbers of predictors
num_predictors <- ncol(reg_summary_df) - 1  # Exclude Intercept column
filtered_models <- lapply((num_predictors - 81 + 1):(num_predictors - 60 + 1), function(n) {
  reg_summary_df[rowSums(reg_summary_df[, -1]) == n, ]
})
