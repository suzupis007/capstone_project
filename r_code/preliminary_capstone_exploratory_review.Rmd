...
library(leaps)               # for resubset selection
...
# calculate VIF (varience inflation factor)
vif_values <- vif(lm(response ~ ., data = qol_numeric))
high_vif <- names(vif_values[vif_values > 5])
high_vif

# no high VIF values identified
```

```{r qol low interest feature removal}
# discussed features with low interest for the model to remove
...
```{r identifying ideal # of variables}
...
# Get the matrix of which variables are included in each model
reg_summary_matrix <- reg_fit_summary$which

# Convert to data frame
reg_summary_df <- as.data.frame(reg_summary_matrix)

# Rename columns to make it more readable
colnames(reg_summary_df) <- sub("X\\d+", "", colnames(reg_summary_df))

# Filter for models with 81 to 60 predictors
num_predictors <- ncol(reg_summary_df) - 1  # Exclude Intercept column
filtered_models <- reg_summary_df[rowSums(reg_summary_df[, -1]) >= (num_predictors - 81 + 1) & rowSums(reg_summary_df[, -1]) <= (num_predictors - 60 + 1), ]
...
```{r % poor to fair vs school funding gap}
# Create the scatter plot with x-axis limit, percentage formatting, and state labels
ggplot(data = qol_data,
...
```{r % poor to fair vs years of life}
# Create the scatter plot with x-axis limit, percentage formatting, and state labels
ggplot(data = qol_data,
...
```{r % vacant houses vs response}
ggplot(qol_data, mapping = aes(x = pct_houses_vacant, y = pct_poor_to_fair_health)) +
  geom_point()
```
